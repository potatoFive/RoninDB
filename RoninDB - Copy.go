...
func roninObjects(w http.ResponseWriter, req *http.Request) {
	...
	if search != "" || wear != "" || (itemType != "" && itemType != "All") || (zone != "" && zone != "All") || (affFlag != "" && affFlag != "All") || (selAff != "" && selAff != "All") || affectMod != "" || extraFlag != "" || minDmg != "" || alignment != "" || (classReq != "" && classReq != "All") || excludeDecay || excludeAntiRent || tinyworldChecked || excludeQuestwear {
		...
		for rows.Next() {
			...
			shortDescEncoded := url.QueryEscape(name.String)
			fmt.Fprintf(w, `
				<tr>
					<td>%s</td>
					<td style="color:#bdc1c6;">%s</td>
					<td><a class="item-link" href="/ronin/Objects/Detail?id=%s&name=%s">%s</a></td>
				...
				</tr>`, clean(zone), id.String, id.String, shortDescEncoded, name.String, clean(wearF), clean(ac), clean(dmg), clean(spell), clean(a0), clean(m0), clean(a1), clean(m1), clean(a2), clean(m2), clean(flags))
		}
