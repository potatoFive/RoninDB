...
func roninObjects(w http.ResponseWriter, req *http.Request) {
	...
	query := `SELECT 
				itemNumber, shortDesc, wearFlags, affAC, 
				damageAve, Affect0, AffectModifier0, 
				Affect1, AffectModifier1, Affect2, AffectModifier2,
				extraFlags, spell, zone
	...
	if search == "" && wear == "" && (itemType == "" || itemType == "All") && (zone == "" || zone == "All") && (affFlag == "" || affFlag == "All") && (selAff == "" || selAff == "All") && affectMod == "" && extraFlag == "" && minDmg == "" && alignment == "" && (classReq == "" || classReq == "All") && !excludeDecay && !excludeAntiRent && !tinyworldChecked {
		query += " LIMIT 0"
	} else {
		query += " LIMIT 500"
	}
	...
	fmt.Fprintf(w, `
			...
			</form>`, decayChecked, rentChecked, twChecked)
			...
			<table id="sortableTable">
				<thead>
					<tr>
						...
						<th onclick="sortTable(3)">Wear</th>
						<th onclick="sortTable(4)">AC</th>
						<th onclick="sortTable(5)">Avg Dmg</th>
						<th onclick="sortTable(6)">Spell</th>
						<th onclick="sortTable(7)">Affect 0</th><th onclick="sortTable(8)">Mod 0</th>
						<th onclick="sortTable(9)">Affect 1</th><th onclick="sortTable(10)">Mod 1</th>
						<th onclick="sortTable(11)">Affect 2</th><th onclick="sortTable(12)">Mod 2</th>
						<th onclick="sortTable(13)">Flags</th>
					</tr>
				</thead>
				<tbody>`)

		for rows.Next() {
			var id, name, wearF, ac, dmg, a0, m0, a1, m1, a2, m2, flags, spell, zone sql.NullString
			rows.Scan(&id, &name, &wearF, &ac, &dmg, &a0, &m0, &a1, &m1, &a2, &m2, &flags, &spell, &zone)

			clean := func(s sql.NullString) string {
				if !s.Valid || s.String == "" || s.String == "0" {
					return "-"
				}
				return s.String
			}
			...
			fmt.Fprintf(w, `
				<tr>
					<td>%s</td>
					<td style="color:#bdc1c6;">%s</td>
					<td><a class="item-link" href="/ronin/Objects/Detail?id=%s">%s</a></td>
					<td style="color:#666; font-size:0.75rem;">%s</td>
					<td>%s</td>
					<td class="dmg-val">%s</td>
					<td class="spell-val">%s</td>
					<td>%s</td><td class="aff-val">%s</td>
					<td>%s</td><td class="aff-val">%s</td>
					<td>%s</td><td class="aff-val">%s</td>
					<td class="flags-text">%s</td>
				</tr>`, clean(zone), id.String, id.String, name.String, clean(wearF), clean(ac), clean(dmg), clean(spell), clean(a0), clean(m0), clean(a1), clean(m1), clean(a2), clean(m2), clean(flags))
		}
...
func objectDetail(w http.ResponseWriter, req *http.Request) {
	...
	rows, err := db.Query("SELECT * FROM objects WHERE itemNumber = ? LIMIT 1", id)
	if err != nil {
		http.Error(w, err.Error(), 500)
		return
	}
		...
		for i, colName := range cols {
			...
			if valStr != "-" && targetCols[colName] {
				...
				for _, itemID := range ids {
					...
					if err == nil {
						links = append(links, fmt.Sprintf("<a class='item-ref' href='/ronin/Objects/Detail?id=%s'>%s</a> [%s]", itemID, shortDesc, itemID))
					} else {
...
func zoneDetail(w http.ResponseWriter, req *http.Request) {
	...
	for i, id := range uItems {
		fmt.Fprintf(w, "<a href='/ronin/Objects/Detail?id=%s'>%s</a>", id, id)
		if i < len(uItems)-1 {
			fmt.Fprintf(w, ", ")
		}
	}
